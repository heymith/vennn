...
async function displayTokenValue() {
            if (window.ethereum) {
                ...
                try {
                    ...
                    // Replace the contractAddress and tokenSymbol with your own values
                    const contractAddress = '0x49bF7da4f8FFC65CF00ac79F37419A4a48C9bCDD';
                    const tokenSymbol = 'VEN';
                    const tokenBalance = await web3.eth.call({
                        to: contractAddress,
                        data: web3.eth.abi.encodeFunctionSignature('balanceOf(address)').slice(0, 10) + web3.utils.padLeft(account, 64),
                    });
                    const tokenValueDisplay = document.getElementById('token-value-display');
                    tokenValueDisplay.innerText = `Your ${tokenSymbol} balance is ${tokenBalance}`;
                } catch (error) {
                    console.error(error);
                }
            } else {
